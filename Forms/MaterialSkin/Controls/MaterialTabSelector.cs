namespace Tarot.Forms.MaterialSkin.Controls
{
    ...
    public class MaterialTabSelector : Control, IMaterialControl
    {
        ...
        private const int TabIndicatorHeight = 4;
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (this.animationManager.IsAnimating())
            {
                ...
                g.FillEllipse(
                    rippleBrush,
                    new Rectangle(
                        this.animationSource.X - rippleSize / 2,
                        this.animationSource.Y - rippleSize / 2,
                        rippleSize,
                        rippleSize));
                g.ResetClip();
                rippleBrush.Dispose();
            ...
            foreach (TabPage tabPage in this.baseTabControl.TabPages)
            {
                ...
                Brush textBrush =
                    new SolidBrush(
                        Color.FromArgb(
                            this.CalculateTextAlpha(currentTabIndex, animationProgress),
                            this.SkinManager.ColorScheme.TextColor));

                g.DrawString(
                    tabPage.Text.ToUpper(),
                    this.SkinManager.RobotoMedium10,
                    textBrush,
                    this.tabRects[currentTabIndex],
            ...
            var y = activeTabPageRect.Bottom - 4;
            var x = previousActiveTabRect.X
                    + (int) ((activeTabPageRect.X - previousActiveTabRect.X) * animationProgress);
            var width = previousActiveTabRect.Width
                        + (int) ((activeTabPageRect.Width - previousActiveTabRect.Width) * animationProgress);

            g.FillRectangle(this.SkinManager.ColorScheme.AccentBrush, x, y, width, TabIndicatorHeight);
        }
            ...
            using (var b = new Bitmap(1, 1))
            {
                using (var g = Graphics.FromImage(b))
                {
                    ...
                    for (var i = 1; i < this.baseTabControl.TabPages.Count; i++)
                    {
                        this.tabRects.Add(
                            new Rectangle(
                                this.tabRects[i - 1].Right,
                                0,
                                TabHeaderPadding * 2
                                + (int)
                                  g.MeasureString(this.baseTabControl.TabPages[i].Text, this.SkinManager.RobotoMedium10)
                                   .Width,
                                this.Height));
                    }
                }

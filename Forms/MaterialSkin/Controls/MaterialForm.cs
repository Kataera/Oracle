    ...
    public class MaterialForm : Form, IMaterialControl
    {
        ...
        protected void OnGlobalMouseMove(object sender, MouseEventArgs e)
        {
            if (!IsDisposed)
            {
                ...
                var new_e = new MouseEventArgs(MouseButtons.None, 0, clientCursorPos.X, clientCursorPos.Y, 0);
                OnMouseMove(new_e);
            }
        }
        ...
        protected override void OnResize(EventArgs e)
        {
            ...
            minButtonBounds = new Rectangle(Width - SkinManager.FormPadding/2 - 3*STATUS_BAR_BUTTON_WIDTH, 0,
                STATUS_BAR_BUTTON_WIDTH, STATUS_BAR_HEIGHT);
            maxButtonBounds = new Rectangle(Width - SkinManager.FormPadding/2 - 2*STATUS_BAR_BUTTON_WIDTH, 0,
                STATUS_BAR_BUTTON_WIDTH, STATUS_BAR_HEIGHT);
            xButtonBounds = new Rectangle(Width - SkinManager.FormPadding/2 - STATUS_BAR_BUTTON_WIDTH, 0,
                STATUS_BAR_BUTTON_WIDTH, STATUS_BAR_HEIGHT);
            ...
            if (m.Msg == WM_LBUTTONDBLCLK)
            {
                MaximizeWindow(!Maximized);
                     ...
                     (statusBarBounds.Contains(PointToClient(Cursor.Position)) ||
                      actionBarBounds.Contains(PointToClient(Cursor.Position))) &&
                     !(minButtonBounds.Contains(PointToClient(Cursor.Position)) ||
                       maxButtonBounds.Contains(PointToClient(Cursor.Position)) ||
                ...
                if (headerMouseDown)
                {
                    ...
                    if (mousePoint.X < Width/2)
                        ...
                        Location = Width - mousePoint.X < previousSize.Width/2
                            ? new Point(Cursor.Position.X - previousSize.Width + Width - mousePoint.X,
